...
export class AionUIDatabase {
   ...
   */
  getApiConfig(): IQueryResult<import('@/common/config/storage').IApiConfig | null> {
    try {
      ...
      // Parse JSON fields
      const config: import('@/common/config/storage').IApiConfig = {
        id: row.id,
        enabled: row.enabled === 1,
   ...
   */
  saveApiConfig(config: Partial<import('@/common/config/storage').IApiConfig>): IQueryResult<boolean> {
    try {
      const now = Date.now();
...
export function getDatabaseSync(): AionUIDatabase {
  if (!dbInstance) {
    throw new Error('Database has not been initialized yet. Call await getDatabase() during startup.');
  }
